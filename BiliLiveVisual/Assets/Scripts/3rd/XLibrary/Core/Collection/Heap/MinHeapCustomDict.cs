...
namespace XLibrary.Collection
{
        ...
        where TDictionary : IDictionary<TKey, int>, new()
    {
        ...
        {
            //将末尾节点的替换的预删除的节点上,然后进行调整
            int count = Count;
            int index = indexInList[key];
            var node = values[index];
            var tair = values[count];

            values.RemoveAt(count);
            indexInList.Remove(key);

            if (values.Count > 1)
            {
                values[index] = new KeyValuePair<TKey, TValue>(tair.Key, node.Value);
                indexInList[tair.Key] = index;

                ChangeValue(tair.Key, tair.Value);
            }
        }
